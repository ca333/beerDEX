        ...
        getprice(){
          if(UserData.findOne({key: "userpass"})){
            ...
            try {
                ...
                try {
                   if(bestprice > 0){
                     TradeData.update({ key: "beerprice" }, { $set: { price: Number(((buf/numcoin * bestprice/numcoin).toFixed(8)*100000000).toFixed(0)) }});
                   }else{
                     TradeData.update({ key: "beerprice" }, { $set: { price: 0 }});
                   }
                } catch(e) {
                    throw new Meteor.Error(e);
                }
